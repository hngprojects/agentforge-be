...
def _build_password_reset_email(email: str, reset_url: str) -> tuple[str, str]:
    """Return (plain-text body, html body) for the password reset email."""
    ttl = settings.PASSWORD_RESET_TOKEN_TTL_MINUTES
    plain = (
        f"Hi,\n\n"
        f"We received a request to reset your Agent Forge password.\n\n"
        f"Click the link below to choose a new password:\n\n"
        f"{reset_url}\n\n"
        f"This link expires in {ttl} minute(s).\n\n"
...
def send_password_reset_email(to_email: str, reset_url: str) -> None:
    plain_body, html_body = _build_password_reset_email(to_email, reset_url)
    ...
    send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(
        to=[{"email": to_email}],
        sender={"name": settings.SMTP_FROM_NAME, "email": settings.SMTP_FROM_EMAIL},
        subject="Reset your Agent Forge password",
        html_content=html_body,
        text_content=plain_body,
    )
